 ...
 */
@Controller
@RequestMapping("admin/zhaoping")
public class ZhaoPingController extends BaseController {
	...
	public ModelAndView getByPage(@PathVariable int currentPage) {
		ModelAndView mv = new ModelAndView("admin/company/zhaopingQ");
	try{
	
		// 初始化分页实体
		Pagination page = initPage(currentPage);
		//page.setPageSize(4);
		mv.addObject("list", zhaoPingService.getByPage(page));
	 ...
	 @RequestMapping(value = DELETE)
	  ...
	  {
		 zhaoPingService.delete(key);
	    return "redirect:/admin/zhaoping/list/1.html";
	  }
	  ...
	  public String update(@PathVariable("id") Long key, Model model)
	  {
		  ...
		  Map<String, Object> entity=zhaoPingService.getById(key);
	...
	@RequestMapping(value = SAVE)
	  public String save(HttpServletRequest request,Model model,String viewType,ZhaoPing zhaoPing)
	  {
		try{
			
			 User user=(User)request.getSession().getAttribute(Constants.SESSION_USER);
			 zhaoPing.setLastUpdateUser(user.getLoginName());
			if(viewType.equals("A")){
				zhaoPing.setLastUpdateUser(user.getLoginName());
				zhaoPingService.save(zhaoPing);
			  }else  if(viewType.equals("U")){						
				  zhaoPing.setLastUpdateUser(user.getLoginName());
				  zhaoPingService.update(zhaoPing);
			  }
